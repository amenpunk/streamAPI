...
app.post("/DeleteStream", async (req,res) => {
    ...
    const snap = await db.collection("Platforms").doc(Mail).collection("List").where("URL", "==", URL).get();
    ...
    if(listSnap.length >= 1){
        ...
        await db.collection("Platforms").doc(Mail).collection("List").doc(ID).delete()
        return res.send({
            status : 1,
        ...
        return res.send({
            status : 0,
            message : "No se pudo eliminar el stream"
        })
